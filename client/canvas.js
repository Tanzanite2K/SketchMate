class CanvasManager {
  ...
  setupCanvas() {
    ...
    // append the temp canvas inside the same container and anchor at 0,0
  ...
  resizeCanvas() {
    ...
    // Reset the scale first to prevent cumulative scaling
  ...
  setupEventListeners() {
    // Use Pointer Events for unified handling and to track the active pointer
  ...
  draw(e) {
    ...
    // For shapes: throttle preview with requestAnimationFrame
    this._lastPreviewPos = pos;
    if (!this._previewRaf) {
      this._previewRaf = requestAnimationFrame(() => {
        this._previewRaf = null;
        const p = this._lastPreviewPos;
        // Clear temp canvas properly (use device pixels cleared in identity transform)
        this.tempCtx.setTransform(1, 0, 0, 1, 0, 0);
